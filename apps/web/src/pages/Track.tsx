...
const STATUS_CHIP: Record<string, string> = {
  ...
  GEOCODED: "bg-canvas text-text-secondary",
    ...
    <div className="min-h-screen bg-canvas">
      ...
      <main className="mx-auto max-w-lg p-4">
        ...
        {!error && !data && (
          <div
            role="status"
            className="flex items-center justify-center gap-2 rounded-xl border border-border bg-surface p-6 text-sm text-text-tertiary"
          >
            <span
              aria-hidden="true"
              className="h-4 w-4 animate-spin rounded-full border-2 border-gris-senal border-t-asfalto"
            />
            Cargando…
          </div>
        )}
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-border bg-surface p-5">
              <div className="flex items-center justify-between gap-3">
                <div className="font-mono text-sm font-semibold text-asfalto">
                  {data.trackingNumber}
                </div>
                <span
                  className={`whitespace-nowrap rounded-full px-2.5 py-1 text-[11px] font-semibold ${
                    STATUS_CHIP[data.status] ?? "bg-canvas text-text-secondary"
                  }`}
                >
              ...
              <div className="mt-3 text-sm">
                ...
                <div className="font-semibold text-asfalto">{data.recipient}</div>
                ...
                <dl className="mt-3 space-y-1 border-t border-border pt-3 text-sm">
                  {data.customProperties.map((cp) => (
                    <div key={cp.id} className="flex justify-between gap-3">
                      ...
                      <dd className="font-medium text-asfalto">{cp.value}</dd>
                    </div>
                  ))}
                </dl>
              )}
              {data.status === "IN_TRANSIT" && data.etaMin !== null && (
                <div className="mt-3 flex items-center gap-2 rounded-lg bg-info-bg px-3 py-2 text-sm font-medium text-asfalto">
              ...
              {data.queuePosition && (
                <div className="mt-3 flex items-center gap-2 rounded-lg bg-canvas px-3 py-2 text-sm font-medium text-asfalto">
              ...
              {data.driverPosition && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="mt-3 flex items-center gap-2 rounded-lg border border-asfalto/25 bg-surface px-3 py-2 text-sm font-medium text-asfalto transition duration-200 ease-brand hover:bg-verde/10 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-asfalto"
                >
              ...
              {data.deliveredAt && (
                <div className="mt-3 flex items-center gap-2 rounded-lg bg-success-bg px-3 py-2 text-sm font-medium text-success">
            ...
            <div className="rounded-xl border border-border bg-surface p-5">
              <div className="mb-3 text-sm font-semibold text-asfalto">
                Historial del envío
              </div>
                      ...
                      <div className="flex flex-col items-center">
                        <span
                          ...
                          className={`mt-1 h-2.5 w-2.5 shrink-0 rounded-full ${
                            current ? "bg-asfalto ring-[3px] ring-verde/60" : "bg-verde"
                          }`}
                        />
                        {!current && <span className="w-px flex-1 bg-border" />}
                      </div>
                      <div className="pb-1">
                        <div className="text-sm font-medium text-asfalto">{e.label}</div>
